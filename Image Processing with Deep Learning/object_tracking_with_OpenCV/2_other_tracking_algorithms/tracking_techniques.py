...
gt.rename(columns={"ceter_y": "center_y"}, inplace=True)
...
while True:
    ...
    if ret:
        ...
        if len(car_gt) != 0:
            ...
            cv2.circle(frame, (center_x, center_y), 2, (0, 180, 255), -1)
        ...
        if initBB is not None:
            ...
            for (i,(o,p)) in enumerate(info):
                text = "{}: {} ".format(o,p)
                cv2.putText(frame, text, (10, H -(i*20)-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 0), 2)
            
        cv2.putText(frame,"Frame Number: " + str(f), (10,30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
...
if len(track_df) != 0:
    ...
    plt.xlabel("frame")
    plt.ylabel("Euclidean distance between gt and track")
